...
Describe 'Write-Done' {
    It 'writes the message prefixed with "[OK  ]"' {
        Write-Done -Message 'Company synced' -InformationVariable info

        $info[0].MessageData.Message | Should -Be '[OK  ] Company synced'
    }

    It 'defaults to green' {
        Write-Done -Message 'Company synced' -InformationVariable info

        $info[0].MessageData.ForegroundColor | Should -Be 'Green'
    }

    It 'honors a custom color' {
        Write-Done -Message 'Company synced' -Color 'White' -InformationVariable info
