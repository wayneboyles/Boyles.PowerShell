...
<#
.SYNOPSIS
    Pester tests for Test-BPSClient.
...
Describe 'Test-BPSClient' {
    ...
    It 'returns $true when a client is registered under the key' {
        Add-BPSClient -Key $script:TestKey -Client ([pscustomobject]@{ Name = 'fake-client' })

        Test-BPSClient -Key $script:TestKey | Should -Be $true
    }

    It 'returns $false when no client is registered under the key' {
